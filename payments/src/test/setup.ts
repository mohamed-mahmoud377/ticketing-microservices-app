...
declare global{ // just doing this to avoid importing it everytime
    var getAJWTCookie: (id?:string)=> string[]
}
...
global.getAJWTCookie =  (id?:string)=>{
   //build a JWT payload. {id , email}
    const payload = {
        id: id || new mongoose.Types.ObjectId().toHexString(),  // here we are building a cookie for a random user means that every time you call the function
